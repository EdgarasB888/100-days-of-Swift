...
class ViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func showImageAnimated() {
        UIView.animate(withDuration: 0, delay: 0, options: [], animations: {
            self.imageView.alpha = 0
        }) { (_) in
            self.imageAnimationHelper()
        }
    }
    
    func imageAnimationHelper() {
    ...
    @IBAction func changeFilterTapped(_ sender: UIButton ) {
        ...
        //for iPad action sheet to appear straight from the button instead of floating in the air
